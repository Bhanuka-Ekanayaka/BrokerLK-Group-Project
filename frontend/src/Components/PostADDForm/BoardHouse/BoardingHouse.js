...
const BoardingHouse = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (form.checkValidity() === false) {
            ...
            try {
                const responsePostData = await axios.post('http://localhost:5001/postadd/boarding-house', formData);
                console.log('Form Data Insert Successfully', responsePostData.data);




                const responseUplodPhotos = await axios.post('http://localhost:5001/postadd/boarding-house/upload', formPhoto, {
                    headers: {
                    ...
                    <Form noValidate validated={validated} onSubmit={handleSubmit}>
                        <Row>
                            ...
                            <Form.Group as={Col} controlId="formGridState">
                                <Form.Label>Rooms<small className="text-muted"> </small></Form.Label>
                                <Form.Control type="number" placeholder="100" value={room} onChange={(e) => setRoom(e.target.value)} min='1' required />
                                <Form.Control.Feedback type="invalid">
                                    Please provide a room count.
                                </Form.Control.Feedback>
                            </Form.Group>
